...
int
lsquic_ID17_parse_packet_in_begin (struct lsquic_packet_in *packet_in,
            ...
            struct packin_parse_state *state)
{
    if (length > 0)
    {
        if (0 == (packet_in->pi_data[0] & 0x80))
            return lsquic_ID17_parse_packet_in_short_begin(packet_in, length,
                                                    is_server, cid_len, state);
        else
            return lsquic_ID17_parse_packet_in_long_begin(packet_in, length,
                                                    is_server, cid_len, state);
    }
    else
        return -1;
...
/* See [draft-ietf-quic-tls-15], Section 4 */
