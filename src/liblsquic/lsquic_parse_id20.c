...
/*
 * lsquic_parse_id20.c -- Parsing functions specific to Internet Draft 20
...
static int
id20_gen_one_varint (unsigned char *, size_t, unsigned char, uint64_t);

static unsigned
id20_one_varint_size (uint64_t);

static void
id20_parse_packet_in_finish (lsquic_packet_in_t *packet_in,
...
static size_t
id20_packout_header_size_long_by_flags (const struct lsquic_conn *lconn,
...
static size_t
id20_packout_header_size_long_by_packet (const struct lsquic_conn *lconn,
...
static size_t
id20_packout_header_size_short (const struct lsquic_conn *lconn,
...
static size_t
id20_packout_max_header_size (const struct lsquic_conn *lconn,
                                enum packet_out_flags flags)
{
    if (lconn->cn_flags & LSCONN_HANDSHAKE_DONE)
        return id20_packout_header_size_short(lconn, flags);
    else
        return id20_packout_header_size_long_by_flags(lconn, HETY_INITIAL, flags);
}
                                                                ...
                                                                size_t bufsz)
{
    ...
    need = id20_packout_header_size_long_by_packet(lconn, packet_out);
    if (need > bufsz)
    {
        errno = EINVAL;
        return -1;
...
static int
id20_gen_reg_pkt_header (const struct lsquic_conn *lconn,
            const struct lsquic_packet_out *packet_out, unsigned char *buf,
                                                                size_t bufsz)
{
...
static void
id20_packno_info (const struct lsquic_conn *lconn,
...
static size_t
id20_packout_size (const struct lsquic_conn *lconn,
                                const struct lsquic_packet_out *packet_out)
{
    size_t sz;
    ...
    if ((lconn->cn_flags & LSCONN_HANDSHAKE_DONE)
        ...
        sz = id20_packout_header_size_short(lconn, packet_out->po_flags);
    else
        sz = id20_packout_header_size_long_by_packet(lconn, packet_out);
...
static int
id20_gen_stream_frame (unsigned char *buf, size_t buf_len,
...
static int
id20_gen_crypto_frame (unsigned char *buf, size_t buf_len,
...
static int
id20_parse_stream_frame (const unsigned char *buf, size_t rem_packet_sz,
...
static int
id20_parse_crypto_frame (const unsigned char *buf, size_t rem_packet_sz,
                                        struct stream_frame *stream_frame)
{
    const unsigned char *const pend = buf + rem_packet_sz;
    const unsigned char *p = buf;
...
static int
id20_parse_ack_frame (const unsigned char *const buf, size_t buf_len,
...
static unsigned
id20_rst_frame_size (lsquic_stream_id_t stream_id, uint64_t final_size)
{
    return id20_one_varint_size(stream_id)   /* Stream ID (i) */
         + sizeof(uint16_t)                  /* Application Error Code (16) */
         + id20_one_varint_size(final_size); /* Final Size (i) */
}


static int
id20_gen_rst_frame (unsigned char *buf, size_t buf_len,
        lsquic_stream_id_t stream_id, uint64_t final_size, uint32_t error_code)
{
    unsigned vbits;
    unsigned char *p;

    if (buf_len < id20_rst_frame_size(stream_id, final_size))
        return -1;
    p = buf;
...
static int
id20_parse_rst_frame (const unsigned char *buf, size_t buf_len,
    lsquic_stream_id_t *stream_id_p, uint64_t *final_size_p, uint32_t *error_code_p)
{
    const unsigned char *p = buf + 1;
    const unsigned char *const end = buf + buf_len;
    uint64_t stream_id, final_size;
    uint16_t error_code;
    int r;
...
static int
id20_parse_stop_sending_frame (const unsigned char *buf, size_t buf_len,
...
static int
id20_gen_stop_sending_frame (unsigned char *buf, size_t len,
                            lsquic_stream_id_t stream_id, uint16_t error_code)
{
    int w;

    w = id20_gen_one_varint(buf, len, 0x05, stream_id);
...
static unsigned
id20_stop_sending_frame_size (lsquic_stream_id_t val)
{
    return id20_one_varint_size(val) + sizeof(uint16_t);
}


static int
id20_parse_new_token_frame (const unsigned char *buf, size_t buf_len,
...
static int
id20_gen_ping_frame (unsigned char *buf, int buf_len)
{
    if (buf_len > 0)
    {
        buf[0] = 0x01;
...
static int
id20_gen_connect_close_frame (unsigned char *buf, size_t buf_len,
...
static int
id20_parse_connect_close_frame (const unsigned char *buf, size_t buf_len,
...
static int
id20_gen_ack_frame (unsigned char *outbuf, size_t outbuf_sz,
...
static size_t
id20_calc_stream_frame_header_sz (lsquic_stream_id_t stream_id,
...
static size_t
id20_calc_crypto_frame_header_sz (uint64_t offset)
{
    return 1    /* Frame type */
         + (1 << vint_val2bits(offset))
         + 1    /* Data len */
...
id20_parse_frame_type (unsigned char byte)
{
...
static int
id20_parse_path_chal_frame (const unsigned char *buf, size_t len,
                                                            uint64_t *chal)
{
    if (len > 9)
    {
...
static int
id20_parse_path_resp_frame (const unsigned char *buf, size_t len,
                                                            uint64_t *resp)
{
    return id20_parse_path_chal_frame(buf, len, resp);
}


void
id20_turn_on_fin (unsigned char *stream_frame_header)
{
...
static unsigned
id20_packno_bits2len (enum packno_bits bits)
{
...
id20_calc_packno_bits (lsquic_packno_t packno,
...
static int
id20_parse_one_varint (const unsigned char *buf, size_t len, uint64_t *val)
{
    int s;
...
static int
id20_gen_one_varint (unsigned char *buf, size_t len,
...
static unsigned
id20_one_varint_size (uint64_t val)
{
    unsigned vbits;
...
static int
id20_gen_blocked_frame (unsigned char *buf, size_t buf_len, uint64_t off)
{
    return id20_gen_one_varint(buf, buf_len, 0x14, off);
}


static int
id20_parse_blocked_frame (const unsigned char *buf, size_t sz, uint64_t *off)
{
    return id20_parse_one_varint(buf, sz, off);
}


static unsigned
id20_blocked_frame_size (uint64_t off)
{
    return id20_one_varint_size(off);
}


static int
id20_parse_max_data (const unsigned char *buf, size_t len, uint64_t *val)
{
    return id20_parse_one_varint(buf, len, val);
}


static int
id20_gen_max_data_frame (unsigned char *buf, size_t len, uint64_t val)
{
    return id20_gen_one_varint(buf, len, 0x10, val);
}


static unsigned
id20_max_data_frame_size (uint64_t val)
{
    return id20_one_varint_size(val);
}


static int
id20_parse_retire_cid_frame (const unsigned char *buf, size_t len,
                                                                uint64_t *val)
{
    return id20_parse_one_varint(buf, len, val);
}
...
static int
id20_gen_retire_cid_frame (unsigned char *buf, size_t len, uint64_t val)
{
    return id20_gen_one_varint(buf, len, 0x19, val);
}


static size_t
id20_retire_cid_frame_size (uint64_t val)
{
    return id20_one_varint_size(val);
}
...
static int
id20_parse_new_conn_id (const unsigned char *buf, size_t len, uint64_t *seqno,
...
static unsigned
id20_two_varints_size (uint64_t vals[2])
{
    unsigned vbits[2];
...
static int
id20_gen_two_varints (unsigned char *buf, size_t len,
...
static int
id20_parse_two_varints (const unsigned char *buf, size_t len, uint64_t *vals[2])
{
    const unsigned char *p = buf;
    const unsigned char *const end = p + len;
    int s;
...
static int
id20_parse_stream_blocked_frame (const unsigned char *buf, size_t len,
                            lsquic_stream_id_t *stream_id, uint64_t *offset)
{
    return id20_parse_two_varints(buf, len,
                                       (uint64_t *[]) { stream_id, offset, });
}


static unsigned
id20_stream_blocked_frame_size (lsquic_stream_id_t stream_id, uint64_t off)
{
    return id20_two_varints_size((uint64_t []) { stream_id, off, });
}


static int
id20_gen_streams_blocked_frame (unsigned char *buf, size_t len,
                                    enum stream_dir sd, uint64_t limit)
{
    return id20_gen_one_varint(buf, len, 0x16 + (sd == SD_UNI), limit);
}


static int
id20_parse_streams_blocked_frame (const unsigned char *buf, size_t len,
                                    enum stream_dir *sd, uint64_t *limit)
{
    int s;

    s = id20_parse_one_varint(buf, len, limit);
    if (s > 0)
    {
        if (buf[0] == 0x16)
            *sd = SD_BIDI;
...
static unsigned
id20_streams_blocked_frame_size (uint64_t limit)
{
    return id20_one_varint_size(limit);
}


static int
id20_gen_stream_blocked_frame (unsigned char *buf, size_t len,
                                    lsquic_stream_id_t stream_id, uint64_t off)
{
    return id20_gen_two_varints(buf, len, 0x15, (uint64_t[]){ stream_id, off, });
}


static int
id20_gen_max_stream_data_frame (unsigned char *buf, size_t len,
                                    lsquic_stream_id_t stream_id, uint64_t off)
{
    return id20_gen_two_varints(buf, len, 0x11, (uint64_t[]){ stream_id, off, });
}


static unsigned
id20_max_stream_data_frame_size (lsquic_stream_id_t stream_id, uint64_t off)
{
    return id20_two_varints_size((uint64_t []) { stream_id, off, });
}
...
static int
id20_parse_max_stream_data_frame (const unsigned char *buf, size_t len,
                                lsquic_stream_id_t *stream_id, uint64_t *off)
{
    return id20_parse_two_varints(buf, len, (uint64_t *[]){ stream_id, off, });
}


static int
id20_parse_max_streams_frame (const unsigned char *buf, size_t len,
                                    enum stream_dir *sd, uint64_t *max_streams)
{
    int s;

    s = id20_parse_one_varint(buf, len, max_streams);
    if (s > 0)
...
static int
id20_gen_max_streams_frame (unsigned char *buf, size_t len,
                                    enum stream_dir sd, uint64_t limit)
{
    return id20_gen_one_varint(buf, len, 0x12 + (sd == SD_UNI), limit);
}


static unsigned
id20_max_streams_frame_size (uint64_t limit)
{
    return id20_one_varint_size(limit);
}
...
static size_t
id20_new_connection_id_frame_size (unsigned seqno, unsigned scid_len)
{
    unsigned bits;
...
int
id20_gen_new_connection_id_frame (unsigned char *buf, size_t buf_sz,
            ...
            const unsigned char *token, size_t token_sz)
{
    ...
    if (buf_sz < id20_new_connection_id_frame_size(seqno, cid->len))
        return -1;
...
const struct parse_funcs lsquic_parse_funcs_id20 =
{
    .pf_gen_reg_pkt_header            =  id20_gen_reg_pkt_header,
    .pf_parse_packet_in_finish        =  id20_parse_packet_in_finish,
    .pf_gen_stream_frame              =  id20_gen_stream_frame,
    .pf_calc_stream_frame_header_sz   =  id20_calc_stream_frame_header_sz,
    .pf_parse_stream_frame            =  id20_parse_stream_frame,
    .pf_parse_ack_frame               =  id20_parse_ack_frame,
    .pf_gen_ack_frame                 =  id20_gen_ack_frame,
    .pf_gen_blocked_frame             =  id20_gen_blocked_frame,
    .pf_parse_blocked_frame           =  id20_parse_blocked_frame,
    .pf_blocked_frame_size            =  id20_blocked_frame_size,
    .pf_rst_frame_size                =  id20_rst_frame_size,
    .pf_gen_rst_frame                 =  id20_gen_rst_frame,
    .pf_parse_rst_frame               =  id20_parse_rst_frame,
    .pf_gen_connect_close_frame       =  id20_gen_connect_close_frame,
    .pf_parse_connect_close_frame     =  id20_parse_connect_close_frame,
    .pf_gen_ping_frame                =  id20_gen_ping_frame,
    .pf_parse_frame_type              =  id20_parse_frame_type,
    .pf_turn_on_fin                   =  id20_turn_on_fin,
    .pf_packout_size                  =  id20_packout_size,
    .pf_packout_max_header_size       =  id20_packout_max_header_size,
    .pf_parse_path_chal_frame         =  id20_parse_path_chal_frame,
    .pf_parse_path_resp_frame         =  id20_parse_path_resp_frame,
    .pf_calc_packno_bits              =  id20_calc_packno_bits,
    .pf_packno_bits2len               =  id20_packno_bits2len,
    .pf_packno_info                   =  id20_packno_info,
    .pf_gen_crypto_frame              =  id20_gen_crypto_frame,
    .pf_parse_crypto_frame            =  id20_parse_crypto_frame,
    .pf_calc_crypto_frame_header_sz   =  id20_calc_crypto_frame_header_sz,
    .pf_parse_max_data                =  id20_parse_max_data,
    .pf_gen_max_data_frame            =  id20_gen_max_data_frame,
    .pf_max_data_frame_size           =  id20_max_data_frame_size,
    .pf_parse_new_conn_id             =  id20_parse_new_conn_id,
    .pf_gen_stream_blocked_frame      =  id20_gen_stream_blocked_frame,
    .pf_parse_stream_blocked_frame    =  id20_parse_stream_blocked_frame,
    .pf_stream_blocked_frame_size     =  id20_stream_blocked_frame_size,
    .pf_gen_max_stream_data_frame     =  id20_gen_max_stream_data_frame,
    .pf_parse_max_stream_data_frame   =  id20_parse_max_stream_data_frame,
    .pf_max_stream_data_frame_size    =  id20_max_stream_data_frame_size,
    .pf_parse_stop_sending_frame      =  id20_parse_stop_sending_frame,
    .pf_gen_stop_sending_frame        =  id20_gen_stop_sending_frame,
    .pf_stop_sending_frame_size       =  id20_stop_sending_frame_size,
    .pf_parse_new_token_frame         =  id20_parse_new_token_frame,
    .pf_new_connection_id_frame_size  =  id20_new_connection_id_frame_size,
    .pf_gen_new_connection_id_frame   =  id20_gen_new_connection_id_frame,
    .pf_parse_retire_cid_frame        =  id20_parse_retire_cid_frame,
    .pf_gen_retire_cid_frame          =  id20_gen_retire_cid_frame,
    .pf_retire_cid_frame_size         =  id20_retire_cid_frame_size,
    .pf_gen_streams_blocked_frame     =  id20_gen_streams_blocked_frame,
    .pf_parse_streams_blocked_frame   =  id20_parse_streams_blocked_frame,
    .pf_streams_blocked_frame_size    =  id20_streams_blocked_frame_size,
    .pf_gen_max_streams_frame         =  id20_gen_max_streams_frame,
    .pf_parse_max_streams_frame       =  id20_parse_max_streams_frame,
    .pf_max_streams_frame_size        =  id20_max_streams_frame_size,
};
