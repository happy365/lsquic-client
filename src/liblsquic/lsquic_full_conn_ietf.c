                                                    ...
                                                    enum lsquic_version *ver)
{
    ...
    conn->ifc_ver_neg.vn_ver  = (ver) ? *ver : highest_bit_set(versions);
...
{
    struct lsquic_packet_out *packet_out;
    const uint16_t error_code = HEC_REQUEST_REJECTED;
    size_t need;
    int w;
    ...
    LSQ_DEBUG("generated %d-byte STOP_SENDING frame (stream id: %"PRIu64", "
        "error code: %"PRIu16")", w, stream_id, error_code);
...
static void
generate_stop_sending_frames (struct ietf_full_conn *conn)
{
...
{
    struct ietf_full_conn *conn = (struct ietf_full_conn *) lconn;
    struct dcid_elem **el, **dce_end, **dces[2];
    int eq;
    /*
     * Find two DCIDs:
    ...
    dce_end = conn->ifc_dces + (sizeof(conn->ifc_dces) /
                                          sizeof(conn->ifc_dces[0]));
    for (el = conn->ifc_dces; (el < dce_end) && !(dces[0] && dces[1]); ++el)
        if (*el)
        {
    ...
    LSQ_DEBUG("prepare to retire DCID seqno %"PRIu32"", (*dces[1])->de_seqno);
    *dces[1] = NULL;
    conn->ifc_send_flags |= SF_SEND_RETIRE_CID;
}
        ...
        struct lsquic_packet_in *packet_in, const unsigned char *p, size_t len)
{
    ...
    if (cce < END_OF_CCES(lconn) &&
                        !LSQUIC_CIDS_EQ(&cce->cce_cid, &packet_in->pi_dcid))
        retire_cid(conn, cce, packet_in->pi_received);
    else if (cce < END_OF_CCES(lconn))
    {
        /* [draft-ietf-quic-transport-19] Section 19.16 */
        ABORT_QUIETLY(0, TEC_PROTOCOL_VIOLATION, "cannot retire CID "
            "seqno=%"PRIu64", for it is used as DCID in the packet",
            seqno);
        return 0;
    }
    else
                                        ...
                                        struct lsquic_packet_in *packet_in)
{
    ...
    if (lsquic_packet_in_is_verneg(packet_in))
    {
        ...
        if (!(LSQUIC_CIDS_EQ(&conn->ifc_conn.cn_cid, &packet_in->pi_dcid)
            && LSQUIC_CIDS_EQ(&conn->ifc_conn.cn_dcid, &packet_in->pi_scid)))
        {
    ...
    LSQ_DEBUG("end of version negotiation: agreed upon %s",
                            lsquic_ver2str[conn->ifc_ver_neg.vn_ver]);
    EV_LOG_VER_NEG(LSQUIC_LOG_CONN_ID,
                    "agreed", lsquic_ver2str[conn->ifc_ver_neg.vn_ver]);
...
ieft_full_conn_ci_get_engine (struct lsquic_conn *lconn)
{
    struct ietf_full_conn *conn = (struct ietf_full_conn *) lconn;
...
static const struct conn_iface ietf_full_conn_iface = {
    ...
    .ci_get_engine           =  ieft_full_conn_ci_get_engine,
...
on_goaway (void *ctx, uint64_t stream_id)
{
    ...
    {
        ABORT_QUIETLY(1, HEC_WRONG_STREAM,
