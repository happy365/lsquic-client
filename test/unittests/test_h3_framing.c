...
static const struct parse_funcs *g_pf = select_pf_by_ver(LSQVER_ID20);
                ...
                unsigned initial_stream_window)
{
    ...
    tobjs->lconn.cn_version = LSQVER_ID20;
    tobjs->lconn.cn_esf_c = &lsquic_enc_session_common_id20;
    tobjs->lconn.cn_pack_size = IQUIC_MAX_IPv4_PACKET_SZ;
...
test_hq_framing (int sched_immed, int dispatch_once, unsigned wsize,
                    int flush_after_each_write, size_t conn_limit, unsigned n_packets)
{
    ...
    init_test_objs(&tobjs, conn_limit ? conn_limit : buf_out_sz, buf_out_sz);
                ...
                for (j = 0; j < sizeof(conn_limits) / sizeof(conn_limits[j]); ++j)
                    for (flush_after_each_write = 0; flush_after_each_write < 2; ++flush_after_each_write)
                        for (k = 0; k < sizeof(n_packets) / sizeof(n_packets[0]); ++k)
                            test_hq_framing(sched_immed, dispatch_once, wsizes[i], flush_after_each_write, conn_limits[j], n_packets[k]);
}
